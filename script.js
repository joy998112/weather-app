...
window.onload = function () {
  fetch(
    "https://api.openweathermap.org/data/2.5/weather?q=" +
      value +
      "&appid=3045dd712ffe6e702e3245525ac7fa38&units=metric"
  )
    .then((res) => res.json())
    .then((data) => {
...
function newlocate(lat, long) {
  fetch(
    "https://api.openweathermap.org/data/2.5/weather?lat=" +
      ...
      "&lon=" +
      long +
      "&appid=3045dd712ffe6e702e3245525ac7fa38&lang=en&units=metric"
  )
    .then((res) => res.json())
    .then((data) => {
      var nameval = data["name"];
...
function search(value) {
  fetch(
    ...
    .then((data) => {
      ...
      document.getElementById(value).innerHTML = `<h1>${convert(
        tempature
      )}°<h6>${value}</h6>${timecon(time)} GST</h1>`;
    });
}
    ...
    .then((data) => {
      ...
      forecastList.forEach((forecast) => {
        ...
        if (forecastTime == "2:30:00 PM") {
          ...
          forecastElement.innerHTML = `<h1>${parseInt(
            temperature
          )}°<h6>${forecastDate}</h6>${weatherDescription}</h1>`;
